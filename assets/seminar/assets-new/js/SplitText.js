...
! function(D, u) {
    ...
    function o(D, u, F, C, i, o, d) {
        var a, h, B, f, A, c, g, x, y, b, v, _, S = e(D),
            m = l("paddingLeft", S),
            w = -999,
            ...
            H = l("fontSize", S) * (u.lineThreshold || .2),
            O = S.textAlign,
            V = [],
            j = [],
            M = [],
        ...
        if (q || I)
            for (a = 0; a < B; a++)((c = (f = A[a]).parentNode === D) || I || G && !z) && (_ = f.offsetTop, q && c && Math.abs(_ - w) > H && ("BR" !== f.nodeName || 0 === a) && (g = [], q.push(g), w = _), I && (f._x = f.offsetLeft, f._y = _, f._w = f.offsetWidth, f._h = f.offsetHeight), q && ((f._isSplit && c || !G && c || z && c || !z && f.parentNode.parentNode === D && !f.parentNode._isSplit) && (g.push(f), f._x -= m, s(f, D, R) && (f._wordEnd = !0)), "BR" === f.nodeName && (f.nextSibling && "BR" === f.nextSibling.nodeName || 0 === a) && q.push([])));
        for (a = 0; a < B; a++)
            if (c = (f = A[a]).parentNode === D, "BR" !== f.nodeName)
                if (I && (y = f.style, z || c || (f._x += f.parentNode._x, f._y += f.parentNode._y), y.left = f._x + "px", y.top = f._y + "px", y.position = "absolute", y.display = "block", y.width = f._w + 1 + "px", y.height = f._h + "px"), !z && G)
                    if (f._isSplit)
                        for (f._next = h = f.nextSibling, f.parentNode.appendChild(f); h && 3 === h.nodeType && " " === h.textContent;) f._next = h.nextSibling, f.parentNode.appendChild(h), h = h.nextSibling;
                    else f.parentNode._isSplit ? (f._parent = f.parentNode, !f.previousSibling && f.firstChild && (f.firstChild._isFirst = !0), f.nextSibling && " " === f.nextSibling.textContent && !f.nextSibling.nextSibling && Q.push(f.nextSibling), f._next = f.nextSibling && f.nextSibling._isFirst ? null : f.nextSibling, f.parentNode.removeChild(f), A.splice(a--, 1), B--) : c || (_ = !f.nextSibling && s(f.parentNode, D, R), f.parentNode._parent && f.parentNode._parent.appendChild(f), _ && f.parentNode.appendChild(p.createTextNode(" ")), "span" === k && (f.style.display = "inline"), V.push(f));
        else f.parentNode._isSplit && !f._isSplit && "" !== f.innerHTML ? j.push(f) : G && !f._isSplit && ("span" === k && (f.style.display = "inline"), V.push(f));
        else q || I ? (f.parentNode && f.parentNode.removeChild(f), A.splice(a--, 1), B--) : z || D.appendChild(f);
    ...
    function d(D, e, F, i) {
        ...
        for (m.insertBefore(S, D), S.textContent = D.nodeValue, m.removeChild(D), a = -1 !== (E = function D(u) {
                ...
                return F
            }(D = S)).indexOf("<"), !1 !== e.reduceWhiteSpace && (E = E.replace(c, " ").replace(A, "")), a && (E = E.split("<").join("{{LT}}")), o = E.length, s = (" " === E.charAt(0) ? y : "") + F(), r = 0; r < o; r++)
            if (d = E.charAt(r), _ && (h = _(E.substr(r), e.specialChars))) d = E.substr(r, h || 1), s += f && " " !== d ? i() + d + "</" + B + ">" : d, r += h - 1;
    ...
    y.version = "3.6.1", D.SplitText = y, D.default = y, "undefined" == typeof window || window !== D ? Object.defineProperty(D, "__esModule", {
        value: !0
    }) : delete D.default
}));